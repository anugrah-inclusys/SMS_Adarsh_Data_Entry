...
async function uploadMiscellaneousFile(studentId, filePath, fileName) {
  ...
  try {
    const res = await axios.post(
      `${API_BASE_URL}/students/miscellaneous-file/${studentId}`,
      form,
      {
        headers: {
          ...form.getHeaders(),
...
async function runMiscellaneousFilesUpload() {
  const studentMap = parseStudentMapping();
  const folders = fs.readdirSync(Miscellaneous_FILES_DIR);

  for (const folderName of folders) {
    const folderPath = path.join(Miscellaneous_FILES_DIR, folderName);
    ...
    for (const file of files) {
      ...
      await uploadMiscellaneousFile(studentData.studentId, filePath, file);
    }
  }

  console.log("🎉 All miscellaneous files processed and uploaded.");
}

module.exports = { runMiscellaneousFilesUpload };
