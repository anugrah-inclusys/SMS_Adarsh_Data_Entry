...
async function uploadSpecialEducationAssessment(row) {
  ...
  const steps = [
    {
      step: 2,
      payload: {
        eyeHandCoordination: row["content.fineMotorSkills.eyeHandCoordination"] || "",
        eyeHandCoordinationDetails: row["content.fineMotorSkills.eyeHandCoordinationDetails"] || "",
    ...
    { step: 7, payload: { preVocationalSkills: row["content.preVocationalSkills"] || "" } },
    {
      step: 8,
      payload: {
  ...
  try {
    ...
    console.error(`❌ Step 10 failed for ${row["Student Name"]}`, err.response?.data || err.message);
  }
