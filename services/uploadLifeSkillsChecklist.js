...
function mapSkills(row, tabsStructure) {
  ...
  tabsStructure.forEach((domain) => {
    ...
    Object.keys(row).forEach((key) => {
      const match = key.match(new RegExp(`life_skills\\.${domain}\\.(.+?)\\.(term1|term2|descriptionTerm1|descriptionTerm2)`));
      if (match) {
        const skill = match[1].trim();
        const field = match[2];
        if (!result[domain][skill]) {
          result[domain][skill] = { term1: '', term2: '', descriptionTerm1: '', descriptionTerm2: '' };
        }
...
async function uploadLifeSkillsChecklist(row, tabsStructure, term = 'term1') {
  ...
  try {
    ...
    console.log(`✅ Created life skills checklist for ${studentId} with ID ${checklistId}`);
  } catch (err) {
    console.error(`❌ Failed to create life skills checklist for ${studentId}`, err.response?.data || err.message);
    return;
  }
  ...
  try {
    ...
    console.error(`❌ Final submission failed for ${checklistId}`, err.response?.data || err.message);
  }
}

async function runLifeSkillsChecklistUpload(filePath = './output/life_skills_checklist_with_ids.csv') {
